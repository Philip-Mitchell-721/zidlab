...
A **learning project**, first and foremost. The goal was to get hands-on with the infrastructure
topics my day job as an application developer doesn't touch: Kubernetes, networking,
...
- It was **bootstrapped from [onedr0p/cluster-template](https://github.com/onedr0p/cluster-template)**,
  ...
  GitOps core (Flux, Cilium, Envoy Gateway, cert-manager, Cloudflare Tunnel, etc.).
  I didn't design those pieces.
- Most of the configuration here was **written with AI assistance**. I decided what to build and used AI to help learn the concepts.
...
| **Kubernetes** | Managed via [talhelper](https://budimanjojo.github.io/talhelper/latest/); node config is declarative |
...
Node addresses and the domain live in `cluster.yaml` / `nodes.yaml`, which are gitignored, as
are all key material and the rendered Talos configs.
...
not in the cluster. DNS needs to keep
resolving when the cluster is down, and Plex hardware transcoding works better on this Intel node compared to the AMD chips in the cluster.
...
For node-level problems, Talos has no SSH, so use the API:
